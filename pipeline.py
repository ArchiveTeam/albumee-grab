...
LUA_SHA1 = get_hash(os.path.join(CWD, 'albumee.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            '--warc-header', 'albumee-dld-script-version: ' + VERSION,
            '--warc-header', ItemInterpolation('albumee-item: %(item_name)s'),
        ...
        if item_type == 'album':
            ...
            wget_args.extend(['--warc-header', 'albumee-album: ' + id_])
            wget_args.append('http://album.ee/album/{}/{}'.format(user, id_))
        else:
...
project = Project(
    ...
    <h2>album.ee <span class="links"><a href="http://album.ee/">Website</a> &middot; <a href="http://tracker.archiveteam.org/albumee/">Leaderboard</a></span></h2>
    '''
)
...
pipeline = Pipeline(
    ...
    GetItemFromTracker('http://%s/%s' % (TRACKER_HOST, TRACKER_ID), downloader,
        VERSION),
    PrepareDirectories(warc_prefix='albumee'),
    WgetDownload(
        WgetArgs(),
        max_tries=1,
