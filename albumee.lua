...
--[[  local match = string.match(url, "^https?://static[0-9]*%.album%.ee/files/")
  if match then
...
wget.callbacks.get_urls = function(file, url, is_css, iri)
  ...
  local a, b = string.match(url, "^(https?://static[0-9]*%.album%.ee/files/.+/)[a-z]+(_[^%./]+%..+)")
  if a and b then
    check(a .. "sq" .. b)
    check(a .. "thumb" .. b)
    check(a .. "norm" .. b)
...
wget.callbacks.httploop_result = function(url, err, http_stat)
  ...
  local match = string.match(url["url"], "^https?://album%.ee/album/[0-9]+/([0-9]+)$")
  if not match then
    match = string.match(url["url"], "^https?://album%.ee/node/([0-9]+)/[0-9]+$")
  end
  if match then
    ids[match] = true
  end
